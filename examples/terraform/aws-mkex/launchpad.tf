...
locals {
  ...
  } if contains(local.mcr_roles, ng.role) && ng.connection == "ssh" }]...)
  ...
  } if contains(local.mcr_roles, ng.role) && ng.connection == "winrm" }]...)

  // decide if we need msr configuration (the [0] is needed to prevent an error of no msr instances exit)
  has_msr = sum(concat([0], [for k, ng in local.nodegroups : ng.count if ng.role == local.msr_role])) > 0
}
