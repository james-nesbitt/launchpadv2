...
func (u *UploadFile) resolve() error {
	...
	if stat.IsDir() {
		slog.Debug(fmt.Sprintf("source %s is a directory, assuming %s/**/*", u.Source, u.Source))
...
func (u *UploadFile) glob(src string) error {
	...
	for _, s := range sources {
		...
		slog.Debug(fmt.Sprintf("glob %s found: %s", abs, s))
		...
		if stat.IsDir() {
			slog.Debug(fmt.Sprintf("%s is a directory", abs))
			continue
		}
